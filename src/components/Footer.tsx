"use client";

import React from "react";
import { motion } from "framer-motion";
import { ArrowUp, Mail } from "lucide-react";

export default function Footer() {
  const handleBackToTop = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    window.scrollTo({
      top: 0,
      behavior: "smooth",
    });
  };

  return (
    <footer className="relative bg-background border-t border-card-border py-12 md:py-16 px-4 md:px-6">
      <div className="max-w-6xl mx-auto flex flex-col items-center text-center relative z-10">
        
        {/* Secondary Info Area */}
        <div className="w-full flex flex-col md:flex-row items-center justify-between gap-8 text-xs font-mono text-muted">
          {/* Copyright & credit */}
          <div className="flex flex-col md:items-start gap-1">
            <span>© {new Date().getFullYear()} MIS DESIGN EXHIBITION.</span>
            <span className="opacity-60">Minimalist Art Gallery Aesthetic.</span>
          </div>

          {/* Social icons */}
          <div className="flex items-center gap-6">
            <a
              href="https://github.com/Madusha360"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:text-foreground transition-colors duration-200"
              aria-label="GitHub"
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-[16px] h-[16px]">
                <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
                <path d="M9 18c-4.51 2-5-2-7-2" />
              </svg>
            </a>
            <a
              href="https://www.linkedin.com/in/madu-isuranga-ab129a234"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:text-foreground transition-colors duration-200"
              aria-label="LinkedIn"
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-[16px] h-[16px]">
                <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
                <rect x="2" y="9" width="4" height="12" />
                <circle cx="4" cy="4" r="2" />
              </svg>
            </a>
            <a
              href="https://wa.me/447829620808"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:text-foreground transition-colors duration-200"
              aria-label="WhatsApp"
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-[16px] h-[16px]">
                <path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z" />
              </svg>
            </a>
            <a
              href="mailto:isurangamadusha476@gmail.com"
              className="hover:text-foreground transition-colors duration-200"
              aria-label="Email"
            >
              <Mail size={16} />
            </a>
          </div>

          {/* Back to top */}
          <button
            onClick={handleBackToTop}
            className="flex items-center gap-2 hover:text-foreground transition-colors duration-200 group uppercase"
          >
            <span>back to top</span>
            <motion.span
              animate={{ y: [0, -3, 0] }}
              transition={{ repeat: Infinity, duration: 1.5, ease: "easeInOut" }}
              className="text-accent group-hover:text-foreground transition-colors"
            >
              <ArrowUp size={14} />
            </motion.span>
          </button>
        </div>
      </div>
    </footer>
  );
}
